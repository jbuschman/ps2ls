...
class mesh_t:
    ...
    def write(self, path):
        ...
        for j in range(0, len(self.vertices)):
            ...
            out.write(str(vertex.x) + " " + str(vertex.y) + " " + str(vertex.z) + " " + str(vertex.u) + " " + str(vertex.v) + "\n")
...
files = glob.glob("C:/Users/Colin Basnett/Desktop/dme_export/*.dme")
...
for file in files:
    ...
    for i in range(0, mesh_count):
        ...
        for j in range(0, vertex_count):
            ...
            vertex = vec3_t()
            vertex.x = struct.unpack("f", buffer[0:4])[0]
            vertex.y = struct.unpack("f", buffer[4:8])[0]
            vertex.z = struct.unpack("f", buffer[8:12])[0]
            #vertex.u = float(struct.unpack("B", buffer[12:13])[0]) / 256.0
            #skip 1?
            #vertex.v = float(struct.unpack("B", buffer[14:15])[0]) / 256.0
            #skip 1?

            mesh.vertices.append(vertex)

        # dmod version 4 stores additional (non-position) data vertex data in a separate block immediately after
        # this possibly compressed UV coordinates for each texture channel on the model
        if dmod_version == 4:
            buffer = f.read(4)
            ...
            f.read(vertex_count * bytes_per_vertex)
        ...
        mesh.write("C:/Users/Colin Basnett/Desktop/" + model.name + "_" + str(i) + ".ply")
